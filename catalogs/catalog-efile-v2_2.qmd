---
title: "990 EFILER Data Catalog (v2.2)"
format:
  html:
    toc: true
    toc-location: body
...
::: {.callout-note title="Release v2.2 (2026-08-11) — additive only"}
This build adds ~500,000 late-arriving filings for tax years 2022-2024
and introduces tax year 2024. It is **purely additive**: no previously
published filings were revised, so analyses built on v2.1 remain valid
as published. Details in the
[release note](https://github.com/UrbanInstitute/nccs-contracts/blob/main/governance/release-notes/efile-v2_2.md);
all data releases are listed on the [data release notes page](release-notes.html).
The prior release remains available: [v2.1 catalog](catalog-efile-v2_1.html).
:::
...
# d <- read.csv( paste0( GH.RAW, "AWS-NCCS-EFILE-V2_2.csv" ) )

d <- read.csv( "aws/AWS-NCCS-EFILE-V2_2.csv" )
keep.these.rows <- grepl( "public/efile_v2_2/", d$Key )
d <- dplyr::filter( d, keep.these.rows  )
d <- dplyr::filter( d, Key != "public/efile_v2_2/COUNT-OF-ROWS-BY-TABLE-AND-FORMTYPE-EFILE_V2_1.CSV" )
```
...
DF_ROWS <- "https://nccs-efile.s3.us-east-1.amazonaws.com/public/efile_v2_2/COUNT-OF-ROWS-BY-TABLE-AND-FORMTYPE-EFILE_V2_1.CSV"
